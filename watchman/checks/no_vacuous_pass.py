...
from ._util import _plural, Result
...
def enforce(results):
    ...
    for r in results:
        if r.status == "PASS" and (r.population is None or r.population < r.floor):
            r.status = "FAIL"
            r.message = (f"passed over {_plural(r.population, 'item')}, floor is {r.floor}. "
...
def audit(results):
    ...
    pop = len(results)
    if undeclared:
        return Result(NAME, "FAIL", f"{_plural(len(undeclared), 'check')} declared no floor: "
                      f"{undeclared}", pop, 1)
    if flipped:
        return Result(NAME, "FAIL", f"{_plural(len(flipped), 'check')} passed over fewer items "
